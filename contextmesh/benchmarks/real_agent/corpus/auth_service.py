...
def verify_token_expiry(token: str) -> str:
    """Check token expiry status.

    Returns the exact status string used by the session layer.
    """
    if _is_expired(token):
        return "TOKEN_EXPIRED_ERR_4471"
    return "TOKEN_VALID"
...
MAX_LOGIN_ATTEMPTS_BEFORE_LOCKOUT = 7
